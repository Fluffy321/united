  ...
  try {
    ...
    for (const shul of MAIN_SHULS) {
      if (!remainingNames.has(shul.name)) {
        const logoUrl = shul.website ? `https://logo.clearbit.com/${shul.website}` : null;
    ...
    return Response.json({ ok: true, deletedCount: toDelete.length, upserted, totalAfter: finalAll.length });
  } catch (err) {
