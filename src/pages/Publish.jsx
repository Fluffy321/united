...
function Header({ step, onBack, onMine }) {
  const stepLabel = { choose: 'Choose', details: 'Details', preview: 'Preview', done: 'Done' }[step];
  return (
    <header className="sticky top-0 z-20 border-b border-slate-200/80 bg-white/95 px-4 pb-3 pt-[max(12px,env(safe-area-inset-top))] backdrop-blur-xl">
      <div className="mx-auto flex max-w-[430px] items-center gap-3">
        ...
        <div className="min-w-0 flex-1"><p className="text-[11px] font-black uppercase tracking-[0.18em] text-blue-600">{stepLabel}</p><h1 className="text-xl font-black tracking-[-0.025em] text-slate-950">Create</h1></div>
        <button type="button" onClick={onMine} className="min-h-11 rounded-[15px] px-3 text-sm font-extrabold text-slate-600">My posts</button>
      </div>
    </header>
  );
}

export function PublishView({ step, draft, expandedGroup, status, errors = {}, result, onAction }) {
  return (
    <div className="min-h-screen bg-[#F6F8FC] text-slate-950">
      ...
      <main className="mx-auto w-full max-w-[430px] px-3.5 pb-[calc(28px+env(safe-area-inset-bottom))] pt-5">
        ...
        {step === 'preview' && draft && <><p className="mb-3 px-1 text-[11px] font-black uppercase tracking-[0.18em] text-slate-500">Preview</p><PublishingPreview draft={draft} />{status === 'error' && <p className="mt-3 rounded-[14px] bg-rose-50 px-3 py-2.5 text-sm font-bold text-rose-700">We couldn’t publish that. Your draft is still here.</p>}<button type="button" disabled={status === 'pending'} onClick={() => onAction('publish')} className="mt-5 flex min-h-14 w-full items-center justify-center gap-2 rounded-[18px] bg-blue-600 text-base font-black text-white shadow-[0_14px_28px_rgba(37,99,235,0.24)] disabled:opacity-60">{status === 'pending' ? <><Loader2 className="h-5 w-5 animate-spin" /> Publishing…</> : 'Publish now'}</button><button type="button" onClick={() => onAction('edit')} className="mt-2 min-h-12 w-full text-sm font-extrabold text-slate-600">Edit details</button></>}
...
export default function Publish() {
  ...
  const [step, setStep] = React.useState(initialType ? 'details' : 'choose');
  ...
  const action = async (name, payload) => {
    ...
    if (name === 'edit') setStep('details');
    if (name === 'back') { if (step === 'choose') navigate(-1); else if (step === 'details') setStep('choose'); else if (step === 'preview') setStep('details'); else navigate('/Feed'); }
    ...
    if (name === 'publish' && status !== 'pending') {
      setStatus('pending');
      try {
        const published = await publishingService.publish(toPublishCommand(draft, submissionKey.current));
        setResult(published); setStatus('success'); setStep('done'); sessionStorage.removeItem(DRAFT_KEY);
      } catch { setStatus('error'); }
    }
  };

  return <PublishView step={step} draft={draft} expandedGroup={expandedGroup} status={status} errors={errors} result={result} onAction={action} />;
}
