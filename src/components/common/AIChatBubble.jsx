...
export default function AIChatBubble({ currentUser, isScrollingDown }) {
  ...
  if (!isOpen) {
    return (
      <button
        onClick={handleOpen}
        className="fixed bottom-28 right-6 w-14 h-14 rounded-full bg-gradient-to-br from-indigo-600 to-purple-600 text-white shadow-lg hover:shadow-xl active:scale-95 transition-all flex items-center justify-center z-40"
      >
  ...
  return (
    <div className="fixed bottom-28 right-6 w-80 bg-white rounded-2xl shadow-2xl flex flex-col z-40 h-96">
      {/* Header */}
      ...
      <div className="border-t border-slate-100 p-3 flex gap-2 flex-shrink-0">
        ...
        <button
          onClick={handleSend}
          disabled={!input.trim() || isLoading || aiThinking}
        ...
        >
          {isLoading || aiThinking ? <Loader2 className="w-3 h-3 animate-spin" /> : <Send className="w-3 h-3" />}
        </button>
      </div>
    </div>
  );
