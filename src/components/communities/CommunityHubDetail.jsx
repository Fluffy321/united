        ...
        {(community.posts || []).map((post) => (
          <article key={post.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="mb-2 flex flex-wrap items-center gap-2">
              <span className="rounded-full bg-slate-950 px-2.5 py-1 text-[11px] font-black text-white">{post.type}</span>
              <span className="text-[11px] font-semibold text-slate-400">{post.meta}</span>
            </div>
            <h3 className="text-[15px] font-black text-slate-950">{post.title}</h3>
            <p className="mt-1 text-sm leading-6 text-slate-600">{post.body}</p>
            <p className="mt-3 text-xs font-bold text-slate-500">Posted by {post.author}</p>
          </article>
        ))}
      </div>
    </div>
