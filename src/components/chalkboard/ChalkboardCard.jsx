...
export default function ChalkboardCard({ post, currentUser, onComment, onMessage, onDelete, onReport }) {
  ...
  return (
    <div className={`bg-white rounded-2xl p-3 shadow-sm border-2 hover:shadow-md transition-shadow ${
      isHelpPost ? 'border-red-200 bg-red-50/20' : 'border-slate-100'
    } ${isExpired ? 'opacity-60' : ''}`}>
      {isDating && post.author_avatar_url && (
        <div className="flex justify-center mb-3 -mt-2">
          <div className="w-20 h-20 rounded-full overflow-hidden border-4 border-pink-100 shadow-lg">
