...
- The PageNotFound trust failure was removed on 2026-09-02; keep every 404 user-facing and free of internal AI or implementation language.
...
8. Update internal/roadmap.js with the shipped screen, evidence, and the next recommended screen; mark this program shipped only when the agreed live route inventory is complete.`,
...

  {
    ...
    shippedNote: 'Shipped 2026-07-29 and simplified in production on 2026-07-31 at commit 348edb3. The root route replaces into the protected /Feed path; /welcome redirects to /login and the former Landing.jsx marketing page is deleted. Direct /login renders the approved Neutral Pearl welcome with market-neutral copy, two ink-framed brand marks, a single centered orbital band, and 56 deterministic irregular cool-white stars; protected-route from_url visits still open sign-in directly. Live verification confirmed https://www.junited.us/login renders “Stay close to what matters,” /welcome resolves to /login, / resolves to /login?from_url=%2FFeed for signed-out visitors, and all three paths produce no browser console errors.',
...
8. Update internal/roadmap.js: change this item's status to 'shipped'.`,
...

  {
    ...
    why: 'Phase 1 deliberately avoided notification overbuild and did not add check-in columns. Build when notification preferences and event operations are mature. Partial progress 2026-07-09 (master-plan item 12): morning-of event reminders for going/maybe/interested RSVPs shipped as supabase/functions/event-reminders + cron 20260709090000, with preference checks and per-(user,event) dedupe. Remaining here: announcement targeting and attendance/check-in.',
...
6. Update internal/roadmap.js: change this item's status to 'shipped'.`,
...

  {
    ...
    why: 'The full plan, the reusable agent prompt, and the per-item progress tracker live in internal/master-plan.md — that file is the source of truth for item status; this entry tracks the initiative as a whole. Individual items that overlap existing roadmap entries (Stripe Connect, orphaned-scaffolding cleanup, etc.) keep their own entries. Progress: item 10 shipped on 2026-07-08 with unified backend user_streaks reads/writes, Profile/FiveTownsBrief backend streak display, and a streak-at-risk evening push cron. Item 11 shipped 2026-07-09: chat tables + conversations added to the supabase_realtime publication via migrations (delivery previously depended on a dashboard toggle), group chat appends realtime rows to the query cache with polling relaxed to a 30s self-heal, DM typing indicators via ephemeral broadcast channels, and DM read receipts ("Seen") off live unread_count row updates. Item 12 shipped same day: DailyTapMitzvah one-tap daily action in the Five Towns Brief feeding the backend streak, plus event-reminders edge function + cron for RSVP morning-of reminders. The Brief was redesigned 2026-07-31 into a compact, market-neutral morning briefing that uses editor-pinned or real feed items only, with mitzvah and explore actions behind optional disclosure panels.',
