...
- [~] 8. React Query migration + giant-file splits — Settings.jsx migrated (commit f3555bd3: ['active-subscription', uid]; BlockedUsersCard on Feed's ['user-blocks', uid] key, unblock invalidates Messages' block/conversation keys). Communities.jsx migrated 2026-07-01 (queries `['communities-list']` — finally a reader for the key CommunityDetailView invalidates — `['community-groups-list']`, `['user-community-ids', uid]`, `['user-group-ids', uid]`; join/leave as useMutation with optimistic cache updates + rollback; localStorage fallback preserved via placeholderData). CommunityAdminCenter.jsx split 3,733 → 134 lines into 13 modules under communities/admin/ (move-only; AppealSubmitModal re-export contract preserved). Remaining: CommunityDetailView.jsx (~2,800 lines) per-tab split — riskier because each tab call-site passes a bespoke prop list rather than a shared spread.
