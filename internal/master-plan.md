...
1. Fix or pull Marketplace: src/pages/Marketplace.jsx stores listings in useState([]) and silently discards them. Wire it to postsService.createMarketplaceListing + a real query with React Query, or remove it from nav/routes until Stripe Connect ships. No silent data loss in production.
...
3. Honesty pass: public entry copy must only describe what works. Fix dead links (PrivacyRights.jsx → /UserSettings, FriendsHub.jsx invite URL → /InviteJoin).
...
20. SEO/shareability: public OG metadata for communities and the signed-out entry; App Store listing assets are already drafted in internal/app-store/.
...
- [x] 3. Public-entry honesty pass + dead links — the former Landing.jsx was audited, then retired entirely on 2026-07-31 when JUnited adopted one signed-out entry. PrivacyRights → /Settings; FriendsHub invite → /login.
...
- [~] 20. SEO/shareability — site-wide half shipped 2026-07-09: index.html now carries og:url/og:site_name/og:image (+dimensions), twitter summary card tags, and a canonical link, so shared junited.us links render preview cards; also removed a Figma html-to-design capture script that was shipping third-party JS to every production page load. Remaining: per-community OG metadata requires serving crawler-visible HTML per route (edge function or prerender) — same SSR blocker as item 19's invite preview cards; do both together in a dedicated session.
