...
- [~] 8. React Query migration + giant-file splits — 2026-07-06 first tranche: PostDetail, Search, PublicProfile migrated from manual useEffect fetching to useQuery (keys: unified-post, user-post-likes, blocked-ids, universal-search, public-profile, is-friend). Biggest file split: CommunityAdminCenter.jsx 3,733 → 1,730 lines, extracting admin-center/{shared,LayoutBrandingTabs,SettingsTab,modals}.jsx (AppealSubmitModal now imported from admin-center/modals). Remaining: Communities.jsx + Settings.jsx + AdminAnalyticsDashboard + JoinByCommunityCode migrations (Communities is tricky — join/leave/reseed handlers mutate the fetched state directly, needs proper mutations+invalidation), and splits for CommunityDetailView (2,814), MitzvahCircle (2,050), MitzvahMap (1,867).
