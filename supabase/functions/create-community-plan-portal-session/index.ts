...
Deno.serve(async (req: Request) => {
  ...
  const { communityId, origin: rawOrigin } = body as {
    communityId?: string;
    origin?: string;
  };
  ...
  const stripeCustomerId = plan?.stripe_customer_id || profile.stripe_customer_id;
  if (!stripeCustomerId) return errorResponse(404, 'No Stripe customer found for this community');
  ...
  try {
    portalSession = await stripe.billingPortal.sessions.create({
      ...
      return_url: `${origin}/Communities?community=${communityId}`,
    });
  } catch (err) {
    console.error('Community plan portal creation failed:', err);
    return errorResponse(500, 'Could not create billing portal session');
  }
